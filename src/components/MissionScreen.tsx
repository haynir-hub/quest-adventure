...
const MissionScreen: React.FC<MissionScreenProps> = ({
  ...
  const isTimerMission =
    !isTextAmount &&
    (mission.missionType === "PLANK" || mission.missionType === "RUN");
    ...
    >
      ...
      <div className="w-full mt-4 md:mt-8 flex flex-col items-center">
        ...
        <p className="text-lg md:text-xl font-bold bg-white/20 p-3 md:p-4 rounded-3xl mx-2 md:mx-4 shadow-inner">
          {theme.missionWrapper
            ? theme.missionWrapper(mission.description, mission.amount)
            : mission.description}
        </p>
      </div>
      ...
      <div className="flex-grow flex flex-col items-center justify-center w-full my-4">
        {isTextAmount ? (
          ...
          <div className="flex flex-col items-center w-full px-4">
            ...
            >
              <span className="text-7xl md:text-8xl font-black mb-1 md:mb-2">
                +
              </span>
      ...
      <div
        className={`w-full h-24 md:h-32 mb-2 md:mb-4 pb-[env(safe-area-inset-bottom)] flex items-center justify-center transition-all duration-500 ${isFinished || isTextAmount ? "opacity-100 translate-y-0" : "opacity-0 translate-y-20 pointer-events-none"}`}
      >
        <button
