...
const getAchievementText = (worldId: string, index: number) => {
    ...
    switch (worldId) {
        case 'mario': return marioAchievements[safeIndex];
        case 'pokemon': return pokemonAchievements[safeIndex];
        case 'treasure': return treasureAchievements[safeIndex];
        default: return 'השגת פריט מיוחד! ✨';
    }
};
...
export const RewardScreen: React.FC<RewardScreenProps> = ({
    ...
    useEffect(() => {
        ...
        const colors = [world.primaryColor, world.secondaryColor, '#ffffff', '#FFD700'];
                ...
                {confetti.map((c) => (
                    <div
                        ...
                        style={{
                            ...
                            borderLeft: c.shape === 'triangle' ? '8px solid transparent' : 'none',
                            borderRight: c.shape === 'triangle' ? '8px solid transparent' : 'none',
                            borderBottom: c.shape === 'triangle' ? `16px solid ${c.color}` : 'none',
                ...
                {isLastMission ? (
                    <>
                        <h1 className="text-3xl md:text-5xl font-black text-gray-800 leading-tight">
                            כל הכבוד!<br />
                            <span style={{ color: world.primaryColor }}>סיימת את ההרפתקה!</span>
                        </h1>
